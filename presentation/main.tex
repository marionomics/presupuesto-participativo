...
\begin{frame}[shrink=15]{Distortion 1: Preference Truncation \textit{(Truncamiento de Preferencias)}}
  \begin{block}{Proposition}
    Under sincere voting, utilitarian social welfare satisfies
    \(W^{\cDEM{\DEM}} \leq W^{\cUAM{\UAM}}\),
    with strict inequality whenever the marginal voter has cross-district preferences.
  \end{block}
  \medskip
  \textbf{Intuition:}
  \begin{itemize}
    ...
    \item Since $\mathcal{M}^{\DEM} \subsetneq \mathcal{M}^{\UAM}$, the UAM always selects from a weakly larger feasible set
  \end{itemize}
\end{frame}
...
\begin{frame}[shrink=15]{Distortion 4: Mobilization Advantage \textit{(Ventaja por Movilización)}}
  \begin{block}{Proposition}
    ...
    \begin{enumerate}[\upshape(i)]
      ...
      \item Causes welfare loss $\mathcal{L}(\mathcal{C}) \geq 0$
    \end{enumerate}
  \end{block}
  \medskip
...
\begin{frame}[shrink=15]{MES Step-by-Step: Example (cont.)}
  ...
  \medskip
  \textbf{Round 2} --- $p_4$: $\mesrho(p_4) = 20$;\quad $p_2$: $\mesrho(p_2) = 18.3$.\\
  $p_3$: total available $= 2 \times 11.67 = 23.3 < 50$ --- \emph{not affordable}.\\
  \textcolor{mesdark}{$\to$ Fund $p_4$ (smallest $\mesrho$).} Deduct 20 from $i=4$.
  Now $\bshare_4 = 5$.

  \medskip
  \textbf{Round 3} --- $p_2$: available $= 11.67 + 5 = 16.67 < 30$ --- not affordable.\\
  $p_3$: not affordable.\quad \textbf{Stop.}

  \medskip
  \textbf{Completion:} Remaining budget $= 40$.
  Fund $p_2$ (cost 30) greedily. Final: $W = \{p_1, p_2, p_4\}$.
...
\begin{frame}[shrink=15]{MES and Geographic Equity \textit{(Equidad Geográfica)}}
  \begin{block}{Proposition (Geographic Equity via EJR)}
    Let $\bdist_d = n_d/n$ be district $d$'s population share.
    Under MES, the funded set $W$ satisfies:
    \[
      \sum_{p \in W \cap \Pset^d} \cost(p) \;\geq\; \bdist_d \cdot \budget
    \]
    for every district with committed residents.
  \end{block}
  \medskip
  \begin{itemize}
    \item Every district gets \textbf{at least its proportional budget share} --- guaranteed
    ...
    \item Can be \textbf{strengthened}: weight $\bshare_i$ by CONAPO marginalization index
      --- voters from high-marginalization \textit{colonias} receive larger shares
  \end{itemize}
\end{frame}
...
\begin{frame}[shrink=15]{An Honest Limitation: MES Is Not Strategy-Proof}
  ...
  \medskip
  \begin{block}{}
    The DEM also fails strategy-proofness (Gibbard-Satterthwaite applies).\\
...
\begin{frame}[shrink=20]{The Property Table}
  ...
  except strategy-proofness --- which no non-dictatorial mechanism can satisfy here.
\end{frame}
...
\begin{frame}[shrink=10]{Policy Recommendations \textit{(Recomendaciones de Política)}}
  \begin{enumerate}
    \item \textbf{Replace the DEM with MES} in Durango's \textit{presupuesto participativo}
      ...
      \end{itemize}
    \item \textbf{Weight budget shares by the CONAPO marginalization index}
      \begin{itemize}
        \item Voters from high-marginalization \textit{colonias} receive larger $\bshare_i$
        \item Strengthens the geographic equity guarantee for the most vulnerable districts
      \end{itemize}
...
\begin{frame}[shrink=20]{Summary \textit{(Resumen)}}
  \begin{enumerate}
    ...
    \item A CONAPO-weighted MES is feasible and improves on the DEM on every dimension
